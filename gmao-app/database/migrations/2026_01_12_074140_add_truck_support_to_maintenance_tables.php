...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // ===== TABLE PREVENTIVE_MAINTENANCES (MP) =====
        if (Schema::hasTable('preventive_maintenances')) {
            Schema::table('preventive_maintenances', function (Blueprint $table) {
                ...
                if (!Schema::hasColumn('preventive_maintenances', 'mileage_interval')) {
                    $table->integer('mileage_interval')->nullable()->after('counter_unit');
                }
                if (!Schema::hasColumn('preventive_maintenances', 'last_mileage')) {
                    $table->integer('last_mileage')->nullable()->after('mileage_interval');
                }
                if (!Schema::hasColumn('preventive_maintenances', 'next_mileage')) {
                    $table->integer('next_mileage')->nullable()->after('last_mileage');
                }
                if (!Schema::hasColumn('preventive_maintenances', 'advance_mileage')) {
                    $table->integer('advance_mileage')->nullable()->default(500)->after('advance_days');
                }
            });
        }
    }
    ...
    public function down(): void
    {
        ...
        // Rollback preventive_maintenances
        if (Schema::hasTable('preventive_maintenances')) {
            Schema::table('preventive_maintenances', function (Blueprint $table) {
                if (Schema::hasColumn('preventive_maintenances', 'truck_id')) {
                    $table->dropForeign(['truck_id']);
            ...
            Schema::table('preventive_maintenances', function (Blueprint $table) {
                $columns = ['asset_type', 'truck_id', 'mileage_interval', 'last_mileage', 'next_mileage', 'advance_mileage'];
                foreach ($columns as $col) {
                    if (Schema::hasColumn('preventive_maintenances', $col)) {
                        $table->dropColumn($col);
                    }
                }
            });
